...
* **Adaptive Admission**: The runner reserves a slot before spawning AGY, using CPU and available memory with a hard maximum of eight workers. It can admit zero new workers under pressure. Configurable via capacity environment variables: `AGY_WORKER_SLOTS`, `AGY_WORKER_MIN`, `AGY_WORKER_MAX` (1..8), `AGY_WORKER_MEMORY_MB` (default 1536), and `AGY_WORKER_CPU_HIGH_PERCENT` (default 90); overrides still obey resource pressure. This behavior is implemented in this branch; no speedup has been measured.
* **Lifecycle Activity**: Safe lifecycle events are persisted and exposed through `agy_job` with `action: "activity"` and optional `args.limit` (default 50). This does not provide private reasoning or a native Codex subagent card. Nested AGY subagents are a bounded prompt allowance; their execution and performance benefit are not guaranteed.
